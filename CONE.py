...
parser.add_argument('--to_evaluate', type=str2bool, default=True)
...
class OpinionClustering():
    ...
    def evaluation(self, sentences, aspect_embeddings, aspect_labels, hotels, dataset="HotelRec"):
        """#pick data
        n_points = input_embeddings.shape[0]
        eva_ids = self.sort_tag(hotels.tolist()[:n_points])
        sentence_ids = eva_ids[hotel_id]
        input_embeddings = np.stack([input_embeddings[i] for i in sentence_ids])
        sentences = [sentences[i] for i in sentence_ids]
        final_result = self.train(self, model, input_embeddings, sentences, initialise=False)
        return final_result"""
    ...
    def result_record(self):
        #here we output the most representative sentence in each cluster
        pass
